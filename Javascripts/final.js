...
const canvas = document.getElementById('finalCanvas'),
      ...
      resetBtn = document.getElementById('resetStickersBtn'),
      retakeModal = document.getElementById('retakeModal'),
      btnRetakeYes = document.getElementById('btnRetakeYes'),
      btnRetakeNo = document.getElementById('btnRetakeNo');
...
if (retakeModal) {
  btnRetakeYes.addEventListener('click', () => window.location.href = 'camera.html');
  btnRetakeNo.addEventListener('click', () => {
    retakeModal.style.animation = 'fadeOut 0.3s forwards';
    setTimeout(() => {
      retakeModal.style.display = 'none';
      retakeModal.style.animation = 'fadeIn 0.3s forwards'; // prepare for next open
    }, 300);
  });
}
...
async function saveToHistory() {
  ...
  try {
    // Coba simpan dengan menyertakan user_email (per-user)
    const { error } = await window.supabase
      .from('photo_history')
      .insert([
        { image_base64: data, user_email: userEmail }
      ]);
      
    if (error) {
      console.warn('Gagal menyimpan dengan user_email, mencoba fallback tanpa user_email:', error.message);
      // Fallback: simpan secara publik jika kolom user_email belum dibuat di database
      ...
      if (fallbackError) {
        console.error('Failed to save to Supabase (fallback):', fallbackError);
      } else {
        console.log('Successfully saved to history (public fallback)');
      }
    } else {
      console.log('Successfully saved to history (per-user)');
    }
  } catch (e) {
